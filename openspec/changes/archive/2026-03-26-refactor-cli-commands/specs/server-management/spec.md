...
- **WHEN** 用户执行 `mcpsmgr install https://docs.bigmodel.cn/cn/coding-plan/mcp/search-mcp-server`
- **THEN** 系统直接将 URL 传递给 GLM5 分析, 提取服务名和配置

#### Scenario: 不支持的输入格式

- **WHEN** 用户执行 `mcpsmgr install @scope/package` 或 `mcpsmgr install bare-name`
...
- **WHEN** 用户在 `mcpsmgr install <url>` 的 GLM 分析确认或环境变量输入 prompt 中按 Ctrl-C
