...
系统 SHALL 支持 `mcpsmgr update [name]` 命令, 根据已记录的 source 信息重新分析并更新中央仓库中的服务配置.

#### Scenario: 更新指定服务

- **WHEN** 用户执行 `mcpsmgr update brave-search` 且该服务 source 不为空且不为 `local`
- **THEN** 系统使用已记录的 source URL 调用 GLM 重新分析, 比对当前 default 配置与新配置的差异, 展示变更预览, 用户确认后更新 `~/.mcps-manager/servers/brave-search.json` 中的 default 和 overrides, 保留原有 env 中用户填写的值

#### Scenario: 更新所有服务

- **WHEN** 用户执行 `mcpsmgr update` 不指定 name
- **THEN** 系统遍历 `~/.mcps-manager/servers/` 中所有服务, 跳过 source 为空或为 `local` 的服务, 依次对有 source 的服务执行更新流程, 最终输出汇总: 更新数, 跳过数, 失败数
...
- **WHEN** GLM 返回新配置, 且新配置的 env 字段包含与旧配置相同的 key
...
- **WHEN** 用户执行 `mcpsmgr update` 且某个服务的 GLM 分析失败 (如网络错误)
- **THEN** 系统记录失败信息, 继续处理下一个服务, 最终汇总中展示失败的服务名和原因
